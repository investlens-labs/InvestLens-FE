...
export function NewsCard({ news }: { news: FeedItem }) {
  ...
  return (
    <article className="surface group p-4 transition hover:border-brand-500/40 hover:shadow-[0_4px_14px_rgba(15,23,42,0.06)]">
      <Link href={`/news/${news.id}`} className="block rounded-md focus-visible:outline-offset-4">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          {primaryImpact ? (
            <>
              <span className="rounded-md bg-slate-900 px-2 py-1 font-mono text-xs font-bold tracking-wide text-white dark:bg-slate-100 dark:text-slate-900">{primaryImpact.ticker}</span>
